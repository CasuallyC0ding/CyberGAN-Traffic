...
latent_dim = 100
batch_size = 128
more_epochs = 50  # Additional epochs to train
...
for epoch in range(start_epoch, start_epoch + more_epochs):
    ...
    print(f"[Epoch {epoch+1}] D Loss: {d_loss_total:.4f} | G Loss: {g_loss_total:.4f}")

    # Optional: save checkpoint after every epoch
    torch.save({
        'epoch': epoch,
...
# Also save final models separately
